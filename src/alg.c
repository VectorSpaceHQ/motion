...
void alg_noise_tune (struct context *cnt, unsigned char *new)
{
	...
	cnt->noise_level = 4 + (cnt->noise_level+sum)/2;
}
...
int alg_diff_standard (struct context *cnt, unsigned char *new)
{
	...
	int i, diffs=0;
	long int level=0;
	int noise_level=cnt->noise_level;
	 ...
	 */
	if (cnt->conf.night_compensate) {
		i=imgs->motionsize;
		for (i--; i>=0; i--) {
			level+=(unsigned char)new[i];
		}
		level/=imgs->motionsize;
		if (level < noise_level * 2)
			noise_level /= 2;
	}
	...
	mmtemp.uw[0] = mmtemp.uw[1] = mmtemp.uw[2] = mmtemp.uw[3] =
		(unsigned short)(noise_level * 255 + 254);
...
/*
	Very fast diff function, does not do night compensation or mask
	overlaying.
*/
