...
void preview_best(struct context *cnt)
{
	...
	if(cnt->preview_max){
		...
		use_jpeg_filename=strcmp(cnt->conf.jpeg_filename, "preview");
	
		if (cnt->ffmpeg_new && !use_jpeg_filename){
...
#endif /* HAVE_FFMPEG */
		/* Save best preview-shot also when no movies are recorded or jpeg_filename
		...
		if (cnt->conf.jpeg_filename)
			...
			jpeg_filename = (char *)DEF_JPEGPATH;
			
		mystrftime(cnt, filename, sizeof(filename), jpeg_filename, cnt->currenttime_tm, NULL, 0);
		sprintf(previewname, "%s/%s.%s", cnt->conf.target_dir, filename, imageext(cnt));
		put_picture(cnt, previewname, cnt->imgs.preview_buffer , FTYPE_IMAGE);
	}
}
