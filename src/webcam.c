...
int http_bindsock(int port, int local)
{
	...
	if (local)
		sockin.sin_addr.s_addr=htonl(INADDR_LOOPBACK);
	else
		sockin.sin_addr.s_addr=htonl(INADDR_ANY);

	setsockopt(sl, SOL_SOCKET, SO_REUSEADDR, &optval, sizeof(optval));

	if (bind(sl, (struct sockaddr *)&sockin, sizeof(struct sockaddr_in))==-1) {
...
static int http_acceptsock(int sl)
{
	...
	if ((sc=accept(sl, (struct sockaddr *)&sockin, &addrlen))>=0) {
		i=1;
		ioctl(sc, FIONBIO, &i);
		return sc;
	}
...
static void webcam_add_client(struct webcam *list, int sc)
{
	...
	static const char header[] = "HTTP/1.0 200 OK\r\n"
			"Server: Motion/" MOTION_VERSION_STRING "\r\n"
			"Connection: close\r\n"
			"Max-Age: 0\r\n"
			"Expires: 0\r\n"
