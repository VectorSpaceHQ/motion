...
/**
 * The context structure is at the very heart of Motion.  It will be
 * defined later in this file, for now we define a pointer type which
 * will be used to reference it (before including any other header
 * files).
 */
typedef struct context *motion_ctxt_ptr;
...
#define MOTION_GREYIMAGE        1
#define MOTION_YUV420PIMAGE    15 
...
/*
 *	To allow plugins to use motion routines without re-compilation,
 *	we define a structure containing pointers to those routines
 *	which may be useful.  This structure will created when the "main"
 *	context is created, and it's pointer will be copied into each
 *	subsequent thread structure.
 */

typedef struct _motion_callbacks {
	void (*motion_log)(int, int, const char *, ...);
	void *(*motion_alloc)(size_t);
	void *(*motion_realloc)(void *, size_t, const char *);
	int (*motion_rotate_map)(struct context *, unsigned char *);
} motion_callbacks;

typedef struct _motion_plugin {
	struct _motion_plugin *next;
	char *name;
	struct mhandle *handle;
...
/*
 * define a struct for the video plugin's working variables.  At the moment,
 * there is only "video_params", but more will probably be needed later.
 */
typedef struct _video_plugin_ctxt {
	void *video_params;
	void *video_ctxt;
} video_plugin_ctxt, *video_plugin_ctxt_ptr;
...
struct context {
	...
	int threadnr;
	int daemon;                   /* set/saved from cmd line */
	int setup_mode;               /* set/saved from cmd line */
	
	int *debug_level;             /* pointer to global var */
	pthread_mutex_t *global_lock; /* pointer to global var */
	volatile int *threads_running;
	pthread_key_t *tls_key_threadnr;
	motion_callbacks *callbacks;   /* for use by plugins */

	struct config conf;
	struct images imgs;
	struct trackoptions track;
	motion_video_input *video_ctxt;
	motion_plugin_ptr motion_video_plugin;
	video_plugin_ctxt *video_vars;
	int precap_nr;
	int precap_cur;
	int new_img;
	int preview_shot;
...
extern motion_plugin_ptr plugins_loaded;
