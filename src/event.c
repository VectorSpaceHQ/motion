...
{
	if (*(int *)devpipe >= 0) {
		if (cnt->video_ctxt->video_pipe_put(
		    cnt, *(int *)devpipe, img, cnt->imgs.size) == -1)
            ...
            void *dummy2 ATTRIBUTE_UNUSED, struct tm *currenttime_tm)
{
	...
	if (conf->output_motion || cnt->new_img==NEWIMG_ON || cnt->preview_shot) {
		const char *jpeg_filename;

		/* conf.jpeg_filename would normally be defined but if someone deleted it by control interface
		   it is better to revert to the default than fail */
		if (cnt->conf.jpeg_filename)
			jpeg_filename = cnt->conf.jpeg_filename;
		else
			jpeg_filename = DEF_JPEGPATH;
			
		mystrftime(cnt, filename, sizeof(filename), jpeg_filename, currenttime_tm, NULL, 0);
		/* motion images gets same name as normal images plus an appended 'm' */
		sprintf(filenamem, "%sm", filename);
		sprintf(fullfilename, "%s/%s.%s", cnt->conf.target_dir, filename, imageext(cnt));
		sprintf(fullfilenamem, "%s/%s.%s", cnt->conf.target_dir, filenamem, imageext(cnt));
	}
	if (conf->output_motion) {
            ...
            void *dummy2 ATTRIBUTE_UNUSED, struct tm *currenttime_tm)
{
	...
	if ( strcmp(cnt->conf.snapshot_filename, "lastsnap") ) {
		char filename[PATH_MAX];
		...
		if (cnt->conf.snapshot_filename)
			snapshot_filename = cnt->conf.snapshot_filename;
		else
			snapshot_filename = DEF_SNAPPATH;
			
		mystrftime(cnt, target_dir, sizeof(target_dir), snapshot_filename, currenttime_tm, NULL, 0);
		sprintf(filename, "%s.%s", target_dir, imageext(cnt));
		sprintf(fullfilename, "%s/%s", cnt->conf.target_dir, filename);
		...
		sprintf(linkpath, "%s/lastsnap.%s", cnt->conf.target_dir, imageext(cnt));
		remove(linkpath);
		...
		sprintf(fullfilename, "%s/lastsnap.%s", cnt->conf.target_dir, imageext(cnt));
		remove(fullfilename);
		put_picture(cnt, fullfilename, img, FTYPE_IMAGE_SNAPSHOT);
	}
            ...
            void *dummy2 ATTRIBUTE_UNUSED, struct tm *currenttime_tm)
{
	...
	mystrftime(cnt, stamp, sizeof(stamp), ffmpeg_filename, currenttime_tm, NULL, 0);

	/* motion mpegs get the same name as normal mpegs plus an appended 'm' */
	sprintf(cnt->motionfilename, "%s/%sm", cnt->conf.target_dir, stamp);
	sprintf(cnt->newfilename, "%s/%s", cnt->conf.target_dir, stamp);

	if (cnt->conf.ffmpeg_cap_new) {
		if (cnt->imgs.type==MOTION_GREYIMAGE) {
		...
		if ( (cnt->ffmpeg_new =
		      ffmpeg_open((char *)cnt->conf.ffmpeg_video_codec, cnt->newfilename, y, u, v,
		                  ...
		                  cnt->conf.ffmpeg_variable_bitrate)) == NULL) {
            ...
            struct tm *currenttime_tm)
{
	...
	if (!cnt->ffmpeg_timelapse) {
		...
		if (cnt->conf.timelapse_filename)
			timelapse_filename = cnt->conf.timelapse_filename;
		else
			timelapse_filename = DEF_TIMEPATH;
		
		mystrftime(cnt, tmp, sizeof(tmp), timelapse_filename, currenttime_tm, NULL, 0);
		sprintf(cnt->timelapsefilename, "%s/%s", cnt->conf.target_dir, tmp);
