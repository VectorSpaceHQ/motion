 ...
 *      netcam_plugin.c
 *
 *      Module for handling network cameras (converted to plugin).
 *
 ...
 */
#define SETUP    ((cnt->conf.setup_mode) || (*cnt->debug_level >= CAMERA_INFO))

#define NETCAM_PARAM ((netcam_config_ptr)cnt->video_vars->video_params)

#include "netcam_params.h"

/* These strings are used for the HTTP connection */
static const char    *connect_req = "GET %s HTTP/1.0\r\n"
                      "User-Agent: Motion-netcam/"
                      MOTION_VERSION_STRING "\r\n"
                      "Connection: close\r\n";
...
static int netcam_read_next_header(netcam_context_ptr netcam)
{
	...
	if (*netcam->cnt->debug_level > CAMERA_INFO)
		netcam->cnt->callbacks->motion_log(-1, 0, "Found image header record");
...
static int netcam_read_first_header(netcam_context_ptr netcam)
{
	...
	if (send(netcam->sock, netcam->connect_request,
		...
		cnt->callbacks->motion_log(LOG_ERR, 1, "Error sending 'connect' request");
		return -1;
	}
	/*
	...
	while (1) {                                 /* 'Do forever' */
		...
		if (SETUP) {
			cnt->callbacks->motion_log(LOG_DEBUG, 0, "Received first header");
		}
		...
		if ((ret = netcam_check_content_type(header)) >= 0) {
			...
			switch (ret) {
				case 1:         /* not streaming */
					if (SETUP) {
						cnt->callbacks->motion_log(LOG_DEBUG, 0,
...
static void netcam_disconnect(netcam_context_ptr netcam)
{
	...
	if (netcam->sock > 0) {
		if (close(netcam->sock) < 0)
			netcam->cnt->callbacks->motion_log(LOG_ERR, 1,
			           "netcam_disconnect");
		netcam->sock = -1;
		rbuf_discard(netcam);
	}
...
static int netcam_connect(netcam_context_ptr netcam, struct timeval *timeout)
{
	...
	if ((setsockopt(netcam->sock, SOL_SOCKET, SO_RCVTIMEO, 
		...
		netcam->cnt->callbacks->motion_log(LOG_ERR, 1, "setsockopt() failed");
		netcam_disconnect(netcam);
		return -1;
	}
	...
	if ((saveflags = fcntl(netcam->sock, F_GETFL, 0)) < 0) {
		netcam->cnt->callbacks->motion_log(LOG_ERR, 1, "fcntl(1) on socket");
		close(netcam->sock);
		netcam->sock = -1;
		return -1;
	}
	...
	if (fcntl(netcam->sock, F_SETFL, saveflags | O_NONBLOCK) < 0) {
		netcam->cnt->callbacks->motion_log(LOG_ERR, 1, "fcntl(2) on socket");
		close(netcam->sock);
		netcam->sock = -1;
		return -1;
	}
	...
	if (fcntl(netcam->sock, F_SETFL, saveflags) < 0) {
		netcam->cnt->callbacks->motion_log(LOG_ERR, 1, "fcntl(3) on socket");
		close(netcam->sock);
		netcam->sock = -1;
		return -1;
	}
	...
	if ((ret < 0) && (back_err != EINPROGRESS)) {
		netcam->cnt->callbacks->motion_log(LOG_ERR, 1, "connect() failed (%d)",
		           back_err);
		close(netcam->sock);
		netcam->sock = -1;
		return -1;
	...
	if (ret == 0) {            /* 0 means timeout */
		netcam->cnt->callbacks->motion_log(LOG_ERR, 0, "timeout on connect()");
		close(netcam->sock);
		netcam->sock = -1;
		return -1;
	}
	...
	if (getsockopt(netcam->sock, SOL_SOCKET, SO_ERROR, &ret, &len) < 0) {
		netcam->cnt->callbacks->motion_log(LOG_ERR, 1, "getsockopt after connect");
		netcam_disconnect(netcam);
		return -1;
	}
	...
	if (ret) {
		netcam->cnt->callbacks->motion_log(LOG_ERR, 0, "connect returned error");
		netcam_disconnect(netcam);
		return -1;
	}
 ...
 * Returns:             Nothing
 */
static void netcam_check_buffsize(netcam_context_ptr netcam, netcam_buff_ptr buff, size_t numbytes)
{
	if ((buff->size - buff->used) >= numbytes)
		return;

	if (*netcam->cnt->debug_level > CAMERA_INFO)
		netcam->cnt->callbacks->motion_log(-1, 0,
		           "expanding buffer from %d to %d bytes",
		           (int) buff->size, (int) buff->size +
		                 NETCAM_BUFFSIZE);

	buff->ptr = netcam->cnt->callbacks->motion_realloc(buff->ptr, buff->size + NETCAM_BUFFSIZE,
...
static int netcam_read_jpeg(netcam_context_ptr netcam)
{
		...
		if (bptr) {
			...
			while (1) {
				...
				if (ptr != netcam->response->buffer) {
					...
					netcam_check_buffsize(netcam, buffer, ix);
	...
	if (netcam->last_image.tv_sec) {
		netcam->av_frame_time = 
		  ...
		  / 10.0;
		if (*netcam->cnt->debug_level > 5)
			netcam->cnt->callbacks->motion_log(-1, 0, "Calculated frame time %f",
			           netcam->av_frame_time);
	}
...
static void *netcam_handler_loop(void *arg)
{
	 ...
	 * thread (necessary for 'cnt->callbacks->motion_log' to function properly).
	 */
	pthread_setspecific(*netcam->cnt->tls_key_threadnr, (void *)((unsigned long)cnt->threadnr));

	if (SETUP) {
		cnt->callbacks->motion_log(LOG_INFO, 0,
	...
	cnt->callbacks->motion_log(LOG_INFO, 0, "netcam camera handler: finish "
	           "set, exiting");
	pthread_exit(NULL);
}
 ...
 *
 */
static void netcam_cleanup(motion_ctxt_ptr ptr)
{
	netcam_context_ptr netcam = (netcam_context_ptr) ptr->video_vars->video_ctxt;
	 ...
	 */
	pthread_mutex_lock(&netcam->mutex);
	if (netcam->cnt->video_vars->video_ctxt == NULL) {
		return;
	}
	/*
	 ...
	 */
	netcam->cnt->video_vars->video_ctxt = NULL;
	/*
	...
	if (pthread_cond_timedwait(&netcam->exiting, &netcam->mutex,
		 ...
		 */
		netcam->cnt->callbacks->motion_log(-1, 0, "No response from camera "
		           "handler - it must have already died");
		pthread_mutex_lock(netcam->cnt->global_lock);
		(*netcam->cnt->threads_running)--;
		pthread_mutex_unlock(netcam->cnt->global_lock);
	}
 ...
 */
static int netcam_next(struct context *cnt, unsigned char *image)
{
	 ...
	 */
	if ((!cnt) || (!cnt->video_vars->video_ctxt))
		return -2;
	netcam = (netcam_context_ptr)cnt->video_vars->video_ctxt;
	/*
...
static int netcam_start(struct context *cnt)
{
	...
	if (*cnt->debug_level > CAMERA_INFO)
		cnt->callbacks->motion_log(-1, 0, "entered netcam_start()");

	memset(&url, 0, sizeof(url));
	if (SETUP) {
		cnt->callbacks->motion_log(LOG_INFO, 0, "Camera thread starting...");
	}
	 ...
	 */
	cnt->video_vars->video_ctxt = (motion_video_input *)cnt->callbacks->motion_alloc(sizeof(struct netcam_context));
	memset(cnt->video_vars->video_ctxt, 0, sizeof(struct netcam_context));
	netcam = (netcam_context_ptr)cnt->video_vars->video_ctxt;           /* Just for clarity in remaining code */
	...
	netcam->response = (struct rbuf *) cnt->callbacks->motion_alloc(sizeof(struct rbuf));
	memset(netcam->response, 0, sizeof(struct rbuf));

	/* Next our image buffers */
	netcam->latest = cnt->callbacks->motion_alloc(sizeof(netcam_buff));
	memset(netcam->latest, 0, sizeof(netcam_buff));
	netcam->latest->ptr = cnt->callbacks->motion_alloc(NETCAM_BUFFSIZE);

	netcam->receiving = cnt->callbacks->motion_alloc(sizeof(netcam_buff));
	memset(netcam->receiving, 0, sizeof(netcam_buff));
	netcam->receiving->ptr = cnt->callbacks->motion_alloc(NETCAM_BUFFSIZE);

	netcam->jpegbuf = cnt->callbacks->motion_alloc(sizeof(netcam_buff));
	memset(netcam->jpegbuf, 0, sizeof(netcam_buff));
	netcam->jpegbuf->ptr = cnt->callbacks->motion_alloc(NETCAM_BUFFSIZE);
	 ...
	 */
	if (NETCAM_PARAM->netcam_proxy) {
		netcam_url_parse(&url, NETCAM_PARAM->netcam_proxy);
		if (!url.host) {
			cnt->callbacks->motion_log(LOG_ERR, 0, "Invalid netcam_proxy (%s)",
			           NETCAM_PARAM->netcam_proxy);
			netcam_url_free(&url);
			return -1;
		}
		if (url.userpass) {
	 ...
	 */
	netcam_url_parse(&url, NETCAM_PARAM->netcam_url);
	if (!url.host) {
		cnt->callbacks->motion_log(LOG_ERR, 0, "Invalid netcam_url (%s)",
		           NETCAM_PARAM->netcam_url);
		netcam_url_free(&url);
		return -1;
	}

	if (NETCAM_PARAM->netcam_proxy == NULL) {
		netcam->connect_host = url.host;
		url.host = NULL;
		netcam->connect_port = url.port;
	}
	 ...
	 * relevance.
	 */
	if (NETCAM_PARAM->netcam_userpass)
		ptr = NETCAM_PARAM->netcam_userpass;
	else
		ptr = url.userpass;
	 ...
	 */
	if (ptr) {
		userpass = cnt->callbacks->motion_alloc(strlen(ptr) + 3);
		strcpy(userpass, ptr);
	} else
		userpass = NULL;
	/*
	...
	if (userpass) {         /* if either of the above are non-NULL */
		...
		encuserpass = cnt->callbacks->motion_alloc(BASE64_LENGTH(strlen(userpass)) + 1);
		/* Fill in the value */
		base64_encode(userpass, encuserpass, strlen(userpass));
		/* Now create the last part (authorization) of the request */
		request_pass = cnt->callbacks->motion_alloc(strlen(connect_auth_req) +
	...
	if (NETCAM_PARAM->netcam_proxy) {
		 ...
		 */
		ptr = cnt->callbacks->motion_alloc(strlen(url.service) + strlen(url.host)
		               + strlen(url.path) + 4);
	 ...
	 */
	netcam->connect_request = cnt->callbacks->motion_alloc(strlen(connect_req) + ix);
	 ...
	 */
	if (setjmp(netcam->setjmp_buffer)) {
		cnt->callbacks->motion_log(LOG_ERR, 0, "libjpeg decompression failure "
	...
	if (netcam->width % 16) {
		cnt->callbacks->motion_log(LOG_ERR, 0,
		           "netcam image width (%d) is not modulo 16",
		           netcam->width);
		return -1;
	}
	if (netcam->height % 16) {
		cnt->callbacks->motion_log(LOG_ERR, 0,
		          "netcam image height (%d) is not modulo 16",
		          netcam->height);
		return -1;
	}
	...
	cnt->imgs.type = MOTION_YUV420PIMAGE;
	...
	pthread_mutex_lock(cnt->global_lock);
	netcam->threadnr = ++(*cnt->threads_running);
	pthread_mutex_unlock(cnt->global_lock);
	if ((retval = pthread_create(&netcam->thread_id,
	                             &handler_attribute,
	                             &netcam_handler_loop, netcam)) < 0) {
		cnt->callbacks->motion_log(LOG_ERR, 1,
...
static void netcam_init(motion_ctxt_ptr cnt) {
	if (cnt->video_vars->video_params) {
		cnt->callbacks->motion_log(LOG_ERR, 0, "in vid_init: video_config already allocated");
	} else {
		cnt->video_vars->video_params = cnt->callbacks->motion_alloc(sizeof(netcam_config));
		memset(cnt->video_vars->video_params, 0, sizeof(netcam_config));
	}
}

static motion_video_input pointer_table = {
        video_init:          netcam_init,
        video_start:         netcam_start,
        video_next:          netcam_next,
        video_close:         NULL,
        video_dev_cleanup:   netcam_cleanup,
        video_cleanup:       NULL,
        video_pipe_start:    NULL,
        video_pipe_put:      NULL
};

static void *netcam_plugin_main(motion_msg msg)
{
	switch (msg) {
		case MOTION_VALIDATION_TABLE:
			return (void *)&netcam_params;      /* in netcam_params.h */
		case MOTION_VALIDATION_SIZE:
			return (void *)&netcam_params_size;
...
motion_plugin_descr MOTION_PLUGIN_SYM = {
        ...
        motion_plugin_control: netcam_plugin_main,
};
