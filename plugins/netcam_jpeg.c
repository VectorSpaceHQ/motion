...
static int netcam_init_jpeg (netcam_context_ptr netcam, j_decompress_ptr cinfo)
{
	...
	if (netcam->imgcnt_last == netcam->imgcnt) {    /* need to wait */
		 ...
		 * rate).  This delay time is in *nanoseconds*.  Note that
		 * here we are assuming framerate is held as an integer, but
		 * we plan to eventually change it to a float (at which time
		 * this code must be enhanced)
		 */
		gettimeofday(&curtime, NULL);
		// I am trying simply with 0.5 sec. Minimum allowed framerate
		// is 2 so we could wait 0.5 sec and still have a good working
		// function. The duplicate image idea is mainly to give a
		// smooth webcam function where clients are accepted and
		// connection is maintained and a rate of 2 is fine for this
		// This is a temporary comment.
		curtime.tv_usec += 500000;
		//curtime.tv_usec += 1000000 / netcam->cnt->conf.frame_limit;
		...
		if (retcode) {    /* we assume a non-zero reply is ETIMEOUT */
			...
			if (*netcam->cnt->debug_level > 3)
				netcam->cnt->callbacks->motion_log(-1,0,"no new pic, no signal rcvd");
			return NETCAM_GENERAL_ERR | NETCAM_NOTHING_NEW_ERR;
		}
		if (*netcam->cnt->debug_level > 3)
			netcam->cnt->callbacks->motion_log(-1, 0, "***new pic delay successful***");
	}
...
/* Working variables */
	...
	if (netcam->cnt->rotate_data.degrees > 0) {
		/* rotate as specified */
		netcam->cnt->callbacks->motion_rotate_map(netcam->cnt, image);
	}
	return netcam->jpeg_error;
}
...
int netcam_proc_jpeg (netcam_context_ptr netcam, unsigned char *image)
{
	...
	if (netcam->width) {    /* 0 means not yet init'ed */
		if ((cinfo.output_width != netcam->width) ||
			...
			netcam->cnt->callbacks->motion_log(LOG_INFO, 0,
