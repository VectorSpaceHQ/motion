 ...
 *
 *       FIXME:  There is quite a bit of room for improvement within this
 * 		module.  There should be a clearly-defined plugin
 * 		initialisation (global), then vid_start and vid_close
 * 		should be called from each thread.  Finally, vid_cleanup
 * 		should take care of the end of the run.  Currently, in
 * 		order to take care of configuration parameters, this is
 * 		not done very cleanly.
 */

/* Common stuff: */
#include "motion.h"
#include "v4l_plugin.h"
#include "plugins.h"
...
#define V4L_PARAM ((v4l_config_ptr)cnt->video_vars->video_params)

#include "v4l_params.h"
...
static void v4l_picture_controls(struct context *cnt, struct video_dev *viddev)
{
	...
	if (cnt->conf.contrast && cnt->conf.contrast != viddev->contrast) {
		...
		if (ioctl(dev, VIDIOCGPICT, &vid_pic)==-1)
			cnt->callbacks->motion_log(LOG_ERR, 1, "ioctl (VIDIOCGPICT)");
	...
	if (make_change) {
		if (ioctl(dev, VIDIOCSPICT, &vid_pic)==-1)
			cnt->callbacks->motion_log(LOG_ERR, 1, "ioctl (VIDIOCSPICT)");
	}
}
                                ...
                                unsigned long freq, int tuner_number)
{
	...
	if (ioctl (dev, VIDIOCGCAP, &vid_caps) == -1) {
		cnt->callbacks->motion_log(LOG_ERR, 1, "ioctl (VIDIOCGCAP)");
		return (NULL);
	}
	...
	if (input != IN_DEFAULT) {
		...
		if (ioctl (dev, VIDIOCGCHAN, &vid_chnl) == -1) {
			cnt->callbacks->motion_log(LOG_ERR, 1, "ioctl (VIDIOCGCHAN)");
		} else {
			vid_chnl.channel = input;
			vid_chnl.norm    = norm;
			if (ioctl (dev, VIDIOCSCHAN, &vid_chnl) == -1) {
				cnt->callbacks->motion_log(LOG_ERR, 1, "ioctl (VIDIOCSCHAN)");
				return (NULL);
			}
		}
	}
	if (freq) {
		...
		if (ioctl (dev, VIDIOCGTUNER, &vid_tuner)==-1) {
			cnt->callbacks->motion_log(LOG_ERR, 1, "ioctl (VIDIOCGTUNER)");
		} else {
			...
			if (cnt->conf.setup_mode)
				cnt->callbacks->motion_log(-1, 0, "Frequency set");
		}
	}
	if (ioctl (dev, VIDIOCGMBUF, &vid_buf) == -1) {
		cnt->callbacks->motion_log(LOG_ERR, 0, "ioctl(VIDIOCGMBUF) - Error device does not support memory map");
		cnt->callbacks->motion_log(LOG_ERR, 0, "V4L capturing using read is deprecated!");
		cnt->callbacks->motion_log(LOG_ERR, 0, "Motion only supports mmap.");
		return NULL;
	} else {
		...
		if (ioctl(dev, VIDIOCMCAPTURE, &vid_mmap) == -1) {
			cnt->callbacks->motion_log(LOG_DEBUG, 1, "Failed with YUV420P, trying YUV422 palette");
			viddev->v4l_fmt=VIDEO_PALETTE_YUV422;
			vid_mmap.format=viddev->v4l_fmt;
			/* Try again... */
			if (ioctl(dev, VIDIOCMCAPTURE, &vid_mmap) == -1) {
				cnt->callbacks->motion_log(LOG_DEBUG, 1, "Failed with YUV422, trying RGB24 palette");
				viddev->v4l_fmt=VIDEO_PALETTE_RGB24;
				vid_mmap.format=viddev->v4l_fmt;
				/* Try again... */
				if (ioctl(dev, VIDIOCMCAPTURE, &vid_mmap) == -1) {
					cnt->callbacks->motion_log(LOG_DEBUG, 1, "Failed with RGB24, trying GREYSCALE palette");
					viddev->v4l_fmt=VIDEO_PALETTE_GREY;
					vid_mmap.format=viddev->v4l_fmt;
					/* Try one last time... */
					if (ioctl(dev, VIDIOCMCAPTURE, &vid_mmap) == -1) {
						cnt->callbacks->motion_log(LOG_ERR, 1, "Failed with all supported palettes "
						                            "- giving up");
						return (NULL);
					}
				}
 ...
 */
static int v4l_next(struct context *cnt, struct video_dev *viddev, unsigned char *map, int width, int height)
{
	int dev=viddev->fd;
	...
	if (ioctl(dev, VIDIOCMCAPTURE, &vid_mmap) == -1) {
		cnt->callbacks->motion_log(LOG_ERR, 1, "mcapture error in proc %d", getpid());
	...
	if (ioctl(dev, VIDIOCSYNC, &vid_mmap.frame) == -1) {
		cnt->callbacks->motion_log(LOG_ERR, 1, "sync error in proc %d", getpid());
		sigprocmask (SIG_UNBLOCK, &old, NULL);
	}
...
static int v4l_open_vidpipe(motion_ctxt_ptr cnt)
{
	int pipe_fd = -1;
	char pipepath[255];
	char buffer[255];
	...
	if (uname(&uts) < 0) {
		cnt->callbacks->motion_log(LOG_ERR, 1, "Unable to execute uname");
		return -1;
	}
	major = strtok(uts.release, ".");
	minor = strtok(NULL, ".");
	if ((major == NULL) || (minor == NULL) || (strcmp(major, "2"))) {
		cnt->callbacks->motion_log(LOG_ERR, 1, "Unable to decipher OS version");
		return -1;
	}
	if (strcmp(minor, "5") < 0) {
		FILE *vloopbacks;
		...
		if (!vloopbacks) {
			cnt->callbacks->motion_log(LOG_ERR, 1, "Failed to open '/proc/video/vloopback/vloopbacks'");
			return -1;
		}

		/* Read vloopback version*/
		if (!fgets(buffer, 255, vloopbacks)) {
			cnt->callbacks->motion_log(LOG_ERR, 1, "Unable to read vloopback version");
			return -1;
		}
		...
		if (!fgets(buffer, 255, vloopbacks)) {
			cnt->callbacks->motion_log(LOG_ERR, 1, "Unable to read vloopback explanation line");
			return -1;
		}
		...
		while (fgets(buffer, 255, vloopbacks)) {
			if (strlen(buffer)>1) {
				...
				if (istatus[0]=='-') {
					...
					if (pipe_fd>=0) {
						cnt->callbacks->motion_log(-1, 0, "\tInput:  /dev/%s", input);
						cnt->callbacks->motion_log(-1, 0, "\tOutput: /dev/%s", output);
						break;
					}
				}
			}
		...
		if ((dir=opendir(prefix))== NULL) {
			cnt->callbacks->motion_log(LOG_ERR, 1, "Failed to open '%s'", prefix);
			return -1;
		}
		...
		closedir(dir);
		if (pipe_fd >= 0) {
			cnt->callbacks->motion_log(-1, 0, "Opened input of %s", pipepath);
		}
	}
	return pipe_fd;
}

static int v4l_startpipe(motion_ctxt_ptr cnt, char *devname, int width, int height, int type)
{
	int dev;
	struct video_picture vid_pic;
	...
	if (ioctl(dev, VIDIOCGWIN, &vid_win)== -1) {
		cnt->callbacks->motion_log(LOG_ERR, 1, "ioctl (VIDIOCGWIN)");
		return(-1);
	}
	vid_win.height=height;
	vid_win.width=width;
	if (ioctl(dev, VIDIOCSWIN, &vid_win)== -1) {
		cnt->callbacks->motion_log(LOG_ERR, 1, "ioctl (VIDIOCSWIN)");
		return(-1);
	}
	return dev;
}

static int v4l_putpipe (motion_ctxt_ptr cnt, int dev, unsigned char *image, int size)
{
 ...
 */
static void vid_init(motion_ctxt_ptr cnt)
{
	...
	if (cnt->video_vars->video_params) {
		...
		cnt->video_vars->video_params = cnt->callbacks->motion_alloc(sizeof(v4l_config));
		memset(cnt->video_vars->video_params, 0, sizeof(v4l_config));
	}
 ...
 */
static void vid_close(motion_ctxt_ptr cnt)
{
	int i=-1;
 ...
 */
static void vid_cleanup(motion_ctxt_ptr cnt)
{
	int i=-1;
	if (viddevs) {
		while(viddevs[++i]) {
 ...
 */
static int vid_start(motion_ctxt_ptr cnt)
{
	...
	width = conf->width;
	height = conf->height;
	input = V4L_PARAM->v4l_input;
	norm = V4L_PARAM->v4l_norm;
	frequency = V4L_PARAM->v4l_frequency;
	tuner_number = V4L_PARAM->v4l_tuner_number;
	...
	while (viddevs[++i]) {
		if (!strcmp(V4L_PARAM->v4l_videodevice, viddevs[i]->video_device)) {
			int fd;
	...
	dev=open(V4L_PARAM->v4l_videodevice, O_RDWR);
	if (dev <0) {
		cnt->callbacks->motion_log(LOG_ERR, 1, "Failed to open video device %s", V4L_PARAM->v4l_videodevice);
		cnt->callbacks->motion_log(LOG_ERR, 0, "Motion Exits");
		exit(1);
	}

	viddevs[i]->video_device=V4L_PARAM->v4l_videodevice;
	viddevs[i]->fd=dev;
 ...
 */
static int vid_next(motion_ctxt_ptr cnt, unsigned char *map)
{
	struct config *conf=&cnt->conf;
	int ret = -1;
...
static int vid_startpipe(motion_ctxt_ptr cnt, const char *devname, int width, int height, int type)
{
	return v4l_startpipe(cnt, (char *)devname, width, height, type);
}

static int vid_putpipe (motion_ctxt_ptr cnt, int dev, unsigned char *image, int size)
{
	return v4l_putpipe(cnt, dev, image, size);
}
...
static motion_video_input pointer_table = {
	...
	video_pipe_start:  vid_startpipe,
	video_pipe_put:    vid_putpipe
};

static void *v4l_plugin_main(motion_msg msg)
{
	switch (msg) {
		case MOTION_VALIDATION_TABLE:
			return (void *)&v4l_params;   /* in v4l_params.h */
		case MOTION_VALIDATION_SIZE:
			return (void *)&v4l_params_size;
...
motion_plugin_descr MOTION_PLUGIN_SYM = {
	...
	motion_plugin_control: v4l_plugin_main,
};
